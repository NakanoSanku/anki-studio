...
const RETENTION_PRESETS = [
  { value: 0.85, label: "轻松", note: "更少复习" },
  { value: 0.9, label: "推荐", note: "平衡模式" },
  { value: 0.92, label: "进阶", note: "更稳记忆" },
  { value: 0.95, label: "冲刺", note: "高频复习" },
] as const
...
function Stepper({
  ...
  return (
    <div className="flex items-center gap-1 rounded-[13px] border border-black/[0.06] bg-card p-1 dark:border-white/[0.08]">
      <button type="button" onClick={() => onChange(Math.max(min, value - step))} className="flex size-8 items-center justify-center rounded-[9px] text-muted-foreground transition-colors hover:bg-muted hover:text-foreground" aria-label={`减少 ${step}${unit}`}>
      ...
      <button type="button" onClick={() => onChange(Math.min(max, value + step))} className="flex size-8 items-center justify-center rounded-[9px] bg-foreground text-background transition-transform active:scale-[0.96]" aria-label={`增加 ${step}${unit}`}>
    ...
    <div className="mx-auto w-full max-w-xl space-y-5 pb-12">
      <section className="rounded-[22px] border border-black/[0.065] bg-card p-5 shadow-[0_18px_46px_-42px_rgba(0,0,0,0.45)] dark:border-white/[0.09] sm:p-6">
        <div className="flex items-center justify-between gap-3">
          ...
          <Button type="button" size="sm" variant="ghost" className="h-8 px-2.5 text-xs text-muted-foreground" onClick={resetDefaults}><RotateCcw className="size-3.5" />恢复默认</Button>
        </div>
        ...
        <h2 className="mt-3 text-xl font-semibold tracking-[-0.035em]">目标保留率</h2>
        ...
        <div className="mt-5 rounded-[16px] border border-black/[0.055] bg-background/55 p-3 dark:border-white/[0.07]">
          ...
          <div className="mt-1 flex justify-between font-mono text-[9px] font-medium text-muted-foreground"><span>70 · 轻负担</span><span>90 · 推荐</span><span>99 · 最牢固</span></div>
        </div>
      </section>

      <section>
        <div className="mb-3 px-1"><p className="text-[10px] font-semibold uppercase tracking-[0.17em] text-muted-foreground">Quick mode</p><h3 className="mt-1 text-xl font-semibold tracking-[-0.035em]">选择学习节奏</h3></div>
      ...
      <section>
        <div className="mb-3 px-1"><p className="text-[10px] font-semibold uppercase tracking-[0.17em] text-muted-foreground">Daily limits</p><h3 className="mt-1 text-xl font-semibold tracking-[-0.035em]">每天学多少</h3></div>
        <div className="overflow-hidden rounded-[20px] border border-black/[0.065] bg-card dark:border-white/[0.09]">
          <div className="flex items-center justify-between gap-3 p-4">
            <div className="min-w-0"><Label className="text-[15px] font-semibold tracking-[-0.02em]">每日新卡</Label><p className="mt-1 text-[11px] text-muted-foreground">每天最多引入多少张陌生卡片</p></div>
            <Stepper value={fsrsSettings.dailyNewLimit} unit="张" min={0} max={999} step={5} onChange={(dailyNewLimit) => update({ dailyNewLimit })} />
          </div>
          <div className="flex items-center justify-between gap-3 border-t border-black/[0.055] p-4 dark:border-white/[0.07]">
            <div className="min-w-0"><Label className="text-[15px] font-semibold tracking-[-0.02em]">每日复习</Label><p className="mt-1 text-[11px] text-muted-foreground">到期卡片每天最多处理多少张</p></div>
            <Stepper value={fsrsSettings.dailyReviewLimit} unit="张" min={0} max={9999} step={20} onChange={(dailyReviewLimit) => update({ dailyReviewLimit })} />
          </div>
          <div className="flex items-center justify-between gap-3 border-t border-black/[0.055] p-4 dark:border-white/[0.07]">
            <div className="min-w-0"><Label htmlFor="max-interval-input" className="text-[15px] font-semibold tracking-[-0.02em]">最长间隔</Label><p className="mt-1 text-[11px] text-muted-foreground">再熟悉的卡也不会超过这个间隔</p></div>
            <div className="relative shrink-0"><Input id="max-interval-input" type="number" min="1" max="36500" value={fsrsSettings.maximumInterval} onChange={(event) => update({ maximumInterval: Math.max(1, Math.min(36500, Number(event.target.value) || 1)) })} className="h-10 w-28 pr-8 text-center font-mono text-xs font-semibold" /><span className="pointer-events-none absolute right-3 top-1/2 -translate-y-1/2 text-[9px] font-medium text-muted-foreground">天</span></div>
          </div>
        </div>
      </section>
    </div>
