...
export function DeckToolsPanel({
  ...
  return (
    <div className="mx-auto w-full max-w-xl space-y-5 pb-12" aria-label="Deck">
      <section className="rounded-[22px] border border-black/[0.065] bg-card p-5 shadow-[0_18px_46px_-42px_rgba(0,0,0,0.45)] dark:border-white/[0.09] sm:p-6">
        ...
        <p className="mt-2 max-w-sm text-sm leading-6 text-muted-foreground">Manage content, templates, imports, and exports in one place.</p>

        <div className="mt-5 grid grid-cols-2 gap-2 border-t border-black/[0.055] pt-4 dark:border-white/[0.07]">
          <div><span className="text-[9px] font-semibold uppercase tracking-[0.14em] text-muted-foreground">Cards</span><p className="mt-1 text-2xl font-semibold tracking-[-0.04em]">{cardCount}</p></div>
          <div className="border-l border-black/[0.055] pl-4 dark:border-white/[0.07]"><span className="text-[9px] font-semibold uppercase tracking-[0.14em] text-muted-foreground">Decks</span><p className="mt-1 text-2xl font-semibold tracking-[-0.04em]">{deckCount}</p></div>
        </div>
      </section>
      ...
      <Link href={PATHS.settingsTemplates} className="group flex items-center gap-4 rounded-[20px] border border-black/[0.065] bg-card p-4 transition-colors hover:bg-muted/45 active:bg-muted/70 dark:border-white/[0.09]">
        ...
        <span className="min-w-0 flex-1">
          ...
          <span className="mt-1 block text-xs text-muted-foreground">Front, back, CSS, and multiple card templates</span>
        </span>
      ...
      <section>
        ...
        <div className="grid grid-cols-2 gap-2.5">
          <button type="button" disabled={actionsDisabled} className="group flex min-h-36 flex-col items-start justify-between rounded-[20px] border border-black/[0.065] bg-card p-4 text-left transition-[background-color,transform] hover:bg-muted/45 active:scale-[0.99] disabled:opacity-50 dark:border-white/[0.09]" onClick={onImport}>
            ...
            <span><span className="block text-lg font-semibold tracking-[-0.035em]">Import</span><span className="mt-1 block text-[11px] leading-4 text-muted-foreground">CSV · JSON · APKG · COLPKG</span></span>
          </button>
          ...
          {exporting ? (
            <button type="button" onClick={onCancelExport} className="relative flex min-h-36 flex-col items-start justify-between overflow-hidden rounded-[20px] bg-foreground p-4 text-left text-background transition-transform active:scale-[0.99]">
              ...
              <span className="relative z-10"><span className="block text-lg font-semibold tracking-[-0.035em]">{exportPercent || "…"}%</span><span className="mt-1 block text-[11px] leading-4 text-background/55">{exportProgress && exportProgress.total > 0 ? `${exportProgress.done} / ${exportProgress.total} · tap to cancel` : "Preparing export · tap to cancel"}</span></span>
            </button>
          ) : (
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <button type="button" disabled={busy} className="group flex min-h-36 w-full flex-col items-start justify-between rounded-[20px] border border-black/[0.065] bg-card p-4 text-left transition-[background-color,transform] hover:bg-muted/45 active:scale-[0.99] disabled:opacity-50 dark:border-white/[0.09]">
                  ...
                  <span className="w-full"><span className="flex items-center justify-between gap-2 text-lg font-semibold tracking-[-0.035em]">Export<ChevronDown className="size-4 text-muted-foreground" /></span><span className="mt-1 block text-[11px] leading-4 text-muted-foreground">APKG · CSV · JSON</span></span>
                </button>
              </DropdownMenuTrigger>
